    ...
    ):Int
    {
        ...
        if(childCount>0)
        {
            for(i in childCount-1 downTo  0)
            {
                ...
                if(vdy>0&&getDecoratedBottom(child!!)-dy<topOffset)
                {
                    removeAndRecycleView(child,recycler)
                    mFirstVisiPos++
                }
                else if(vdy<0&&getDecoratedTop(child!!)-dy>height-paddingBottom)
                {
                    removeAndRecycleView(child,recycler)
                    mLastVisiPos--
                }
        ...
        if(vdy>=0)
        {
            ...
            for(i in minPos .. mLastVisiPos)
            {
                ...
                layoutDecoratedWithMargins(child, leftOffset, topOffset, leftOffset + w, topOffset + h)
                if(topOffset-vdy>height-paddingBottom)
                {
                    removeAndRecycleView(child,recycler)
                    mLastVisiPos=i-1
                }
...
//                    leftOffset+=h
        ...
        else
        {
            ...
            for(i in maxPos downTo mFirstVisiPos)
            {
                ...
                val w=getDecoratedMeasurementHorizontal(child)
                val h=getDecoratedMeasurementVertical(child)
                layoutDecoratedWithMargins(child, leftOffset, bottomOffset-h, leftOffset + w, bottomOffset)
                if(bottomOffset-vdy<paddingTop)
                {
                    removeAndRecycleView(child,recycler)
                    mFirstVisiPos=i+1
                }
